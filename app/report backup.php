    ...
    $app->group('/api', function (Group $group) {
        ...
        $group->get('/report/monthly-closing-init', function (Request $request, Response $response) {
            ...
            try {
                ...
                // 🔍 1. ดึงยоดยกมาจากงบเดือนก่อนหน้า (ตารางแม่) - ดึงมาใช้เป็นกรณีตั้งต้นเสมอ
                ...
                if ($status !== 'none' && $currentClosing) {
                    ...
                    $stmtGetLogs = $db->prepare("
                                SELECT l.action_status, l.note, l.created_at, u.firstname as actor_name
        ...
        $group->post('/monthly-closing/submit', function (Request $request, Response $response) {
            ...
            try {
                ...
                if ($existingId) {
                    ...
                    $sqlMain = "INSERT INTO monthly_closings (church_id, year, month, status, carry_forward, total_income, total_expense, book_balance, note_by_treasurer, submitted_by, submitted_at) 
                        VALUES (:church_id, :year, :month, 'pending', :carry_forward, :total_income, :total_expense, :book_balance, :note, :user_id, NOW())";
                    $stmtMain = $db->prepare($sqlMain);
                    $stmtMain->execute([
                        ':church_id' => $churchId,
                        ':year' => $year,
                ...
                if (!empty($nextApprovers)) {
                    ...
                    foreach ($nextApprovers as $approver) {
                        ...
                        $subject = "แจ้งเตือน: มีเอกสารรอการอนุมัติของคุณ";
                        // $verifyUrl = $_ENV['APP_URL'] . '/verify-email?token=234gsgbsrdss';
                        // <p><a href='{$verifyUrl}'>ยืนยันบัญชีผู้ใช้งาน</a></p>
                        $mailBody = "
                            <p>เรียนคุณ {$firstname}</p>
                            <p>กรุณากดลิงก์ด้านล่างเพื่ออนุมัติการปิดบัญชีรายเดือน</p>
                            <p>โปรดอย่าส่งต่อ Email นี้ให้ผู้อื่น</p>
                        ";

                        try {
                            //sendEmail('boy.thanaphon@gmail.com', $firstname, $subject, $mailBody);
        ...
        $group->post('/monthly-closing-approve-action', function (Request $request, Response $response) {
            ...
            try {
                ...
                $stmtLog = $db->prepare("
                    INSERT INTO monthly_closing_logs (closing_id, action_status, actor_id, note, created_at) 
                    VALUES (:closing_id, :status, :actor_id, :note, NOW())
                ");
                $stmtLog->execute([
