    ...
    $app->group('/api', function (Group $group) {
        ...
        $group->post('/transactions', function (Request $request, Response $response) {
            ...
            try {
                ...
                $filePaths = handleFileUploads($files, $transactionId, $data['church_id'], $data['transaction_date'], $db);
        ...
        $group->post('/transaction/{id}/update', function (Request $request, Response $response, array $args) {
            ...
            try {
                ...
                if (!empty($files)) {
                    ...
                    handleFileUploads($files, $id, $c_id, $t_date, $db);
                }
        ...
        $group->get('/transaction/{id}', function (Request $request, Response $response, array $args) {
            ...
            try {
                ...
                if ($result) {
                    $stmtFiles = $db->prepare("SELECT id, transaction_id, file_name, file_path FROM transaction_attachments WHERE transaction_id = :id");
        ...
        $group->get('/transactions/search', function (Request $request, Response $response) {
            ...
            // 🆕 เพิ่มเงื่อนไขตรวจสอบการระบุ บัญชีธนาคาร (t.bank_account) สำหรับค้นหา
        ...
        $group->post('/transaction/upload-preview', function (Request $request, Response $response) {
            ...
            try {
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    $catId = null; $missionId = null; $departmentId = null; $channelId = null;

                    if (empty($catName)) { $isError = true; $errorReasons[] = 'ไม่มีข้อมูลหมวดหมู่'; }
                    if (empty($channelName)) { $isError = true; $errorReasons[] = 'ไม่มีข้อมูลช่องทาง'; }
                    if (!is_numeric($value)) { $isError = true; $errorReasons[] = 'จำนวนเงินไม่ถูกต้องหรือไม่เป็นตัวเลข'; }

                    if (!empty($catName)) {
                        if (isset($masterCatMap[$catName])) { $catId = $masterCatMap[$catName]; } 
                        else { $isError = true; $errorReasons[] = "หมวดหมู่ '{$catName}' ไม่มีในระบบ"; }
                    }
                    if (!empty($mission)) {
                        if (isset($masterMissionMap[$mission])) { $missionId = $masterMissionMap[$mission]; } 
                        else { $isError = true; $errorReasons[] = "Mission '{$mission}' ไม่มีในระบบ"; }
                    }
                    if (!empty($department)) {
                        if (isset($masterDeptMap[$department])) { $departmentId = $masterDeptMap[$department]; } 
                        else { $isError = true; $errorReasons[] = "Department '{$department}' ไม่มีในระบบ"; }
                    }
                    if (!empty($channelName)) {
                        if (isset($masterChannelMap[$channelName])) { $channelId = $masterChannelMap[$channelName]; } 
                        else { $isError = true; $errorReasons[] = "ช่องทาง '{$channelName}' ไม่มีในระบบ"; }
                    }
                ...
                $errorRows  = count(array_filter($previewItems, function ($item) { return $item['is_error']; }));
                $totalValue = array_reduce($previewItems, function ($sum, $item) { return $sum + ($item['is_error'] ? 0 : $item['value']); }, 0);
